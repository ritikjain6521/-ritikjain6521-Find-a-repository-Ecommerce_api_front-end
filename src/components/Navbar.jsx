      ...
      <div className="nav sticky-top">
        <div className="nav_bar">
          ...
          {location.pathname.includes("/admin") ? (
            <Link to={"/admin"} className="left" style={{ textDecoration: "none", color: "white" }}>
              Admin Panel
            </Link>
          ) : (
          ...
          {/* Desktop right links */}
          <div className="right d-flex flex-wrap justify-content-center align-items-center gap-2 desktop-menu">
            {!location.pathname.includes("/admin") && (
              <>
                {!IsAuthenticated && (
                  <>
                    <Link to={"/login"} className="btn btn-info mx-2">Login</Link>
                    <Link to={"/register"} className="btn btn-secondary mx-2">SignUp</Link>
                  </>
                )}
                ...
                {IsAuthenticated && (
                  <>
                    ...
                    <Link to={"/admin"} className="btn btn-info mx-2" style={{ fontWeight: "bold" }}>Admin</Link>
                  </>
                )}
              </>
            )}
            ...
            {location.pathname.includes("/admin") && (
              <>
                <Link to={"/"} className="btn btn-info mx-2">Go To Home</Link>
        ...
        <div className={`mobile-menu ${menuOpen ? 'mobile-menu-open' : ''}`}>
          {!location.pathname.includes("/admin") && (
            <>
              {!IsAuthenticated && (
                <>
                  <Link to={"/login"} className="btn btn-info mobile-menu-item" onClick={closeMenu}>Login</Link>
                  <Link to={"/register"} className="btn btn-secondary mobile-menu-item" onClick={closeMenu}>SignUp</Link>
                </>
              )}

              {IsAuthenticated && (
                <>
                  <Link to={"/cart"} className="btn btn-primary mobile-menu-item position-relative" onClick={closeMenu}>
                    🛒 Cart
                    {cart?.items?.length > 0 && (
                      <span className="badge rounded-pill bg-danger ms-2">{cart?.items?.length}</span>
                    )}
                  </Link>
                  <Link to={'/Profile'} className="btn btn-info mobile-menu-item" onClick={closeMenu}>Profile</Link>
                  <button className="btn btn-danger mobile-menu-item" onClick={() => { Logout(); navigate('/'); closeMenu(); }}>Logout</button>
                  <Link to={"/admin"} className="btn btn-info mobile-menu-item" style={{ fontWeight: "bold" }} onClick={closeMenu}>Admin</Link>
                </>
              )}
            </>
          )}
          ...
          {location.pathname.includes("/admin") && (
            <>
              <Link to={"/"} className="btn btn-info mobile-menu-item" onClick={closeMenu}>Go To Home</Link>
        ...
        {location.pathname == "/" && (
          <div className="sub_br">
            ...
            <div className="items" onClick={() => filterbyCategory("leptop")}>Laptop</div>
            <div className="items" onClick={() => filterbyCategory("HP Leptop bug")}>Laptop Begg</div>
